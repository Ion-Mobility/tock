...
pub const DMA_TCD_COUNT: usize = 16;
...
const DMA_MUX_BASE: StaticRef<DmaMultiplexerRegisters> =
    unsafe { StaticRef::new(0x4002_1000 as *const DmaMultiplexerRegisters) };
...
/// register.
#[repr(transparent)]
struct ChannelPriorityRegisters([ReadWrite<u8, ChannelPriority::Register>; DMA_DCHPRI_COUNT]);
...
const DMA_BASE: StaticRef<DmaRegisters> =
    unsafe { StaticRef::new(0x4000_8000 as *const DmaRegisters) };
...
impl<'a> Dma<'a> {
    ...
    pub const fn new(pcc: &'a pcc::Pcc) -> Self {
        Dma {
            ...
            clock_gate: pcc::PeripheralClock::new(pcc, pcc::ClockGate::PccFTFCGate),
            registers: DMA_BASE,
        }
    }
...
const DMA_CHANNELS: [DmaChannel; DMA_DCHPRI_COUNT] = [
    DmaChannel::new(0),
    DmaChannel::new(1),
    DmaChannel::new(2),
    DmaChannel::new(3),
